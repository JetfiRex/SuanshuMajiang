...
# 导入番数计算器
try:
    from fan_calculator.fan_calculator import calculate_fan, format_fan_result
...
# 导入空听检查模块
try:
    from calculator_base.empty_listening_checker import analyze_ready_tiles, format_ready_tiles_with_status
    EMPTY_LISTENING_AVAILABLE = True
except ImportError:
    EMPTY_LISTENING_AVAILABLE = False
    print("警告: 未找到 empty_listening_checker.py，空听检查功能将不可用")
    # 空听检查是可选功能，不需要警告

# 导入特殊胡法模块
try:
    from calculator_base.special_winning_checker import SpecialWinningChecker
    SPECIAL_WINNING_AVAILABLE = True
except ImportError:
    SPECIAL_WINNING_AVAILABLE = False
    print("警告: 未找到 special_winning_checker.py，特使胡法功能将不可用")
...
class ArithmeticMahjong:
    ...
    def is_ready(self, hand, return_details=False):
        ...
        if hand_len == 15:
            ...
            # 3. 检查特殊胜利听牌（十三幺、天龙、地龙）
            if self.special_winning_checker is not None:
                # 检查十三幺听牌
                is_ready_13yao, tiles_13yao = self.special_winning_checker.is_ready_shi_san_yao(hand)
                if is_ready_13yao:
                    if return_details:
                        yao13_details = {}
                        for tile in tiles_13yao:
                            test_hand = hand + [tile]
                            can_win, _ = self.special_winning_checker.can_win_shi_san_yao(test_hand)
                            if can_win:
                                fan_info = self._calculate_fan(test_hand, [], "十三幺", None)
                                yao13_details[tile] = {
                                    'groups': [],
                                    'win_type': '十三幺',
                                    'fan_info': fan_info
                        ...
                        ready_info['十三幺'] = {
                            'tiles': sorted(tiles_13yao, key=lambda x: (x not in SYMBOLS, x)),
                            'details': yao13_details
                        }
                    else:
                        # 使用空听检查模块分析听牌状态
                        if EMPTY_LISTENING_AVAILABLE:
                            analysis = analyze_ready_tiles(list(tiles_13yao), hand)
                            annotated_tiles = []
                            for tile in sorted(tiles_13yao, key=lambda x: (x not in SYMBOLS, x)):
                                if tile in analysis:
                                    annotated_tiles.append(analysis[tile]['display'])
                                else:
                                    annotated_tiles.append(str(tile))
                            ready_info['十三幺'] = annotated_tiles
                        else:
                            ready_info['十三幺'] = sorted(tiles_13yao,
                                                          key=lambda x: (x not in SYMBOLS, x))
                
                # 检查天龙听牌
                is_ready_tl, tiles_tl = self.special_winning_checker.is_ready_tian_long(hand)
                if is_ready_tl:
                    if return_details:
                        tl_details = {}
                        for tile in tiles_tl:
                            test_hand = hand + [tile]
                            can_win, _ = self.special_winning_checker.can_win_tian_long(test_hand)
                            if can_win:
                                fan_info = self._calculate_fan(test_hand, [], "天龙", None)
                                tl_details[tile] = {
                                    'groups': [],
                                    'win_type': '天龙',
                                    'fan_info': fan_info
                        ...
                        ready_info['天龙'] = {
                            'tiles': sorted(tiles_tl, key=lambda x: (x not in SYMBOLS, x)),
                            'details': tl_details
                        }
                    else:
                        # 使用空听检查模块分析听牌状态
                        if EMPTY_LISTENING_AVAILABLE:
                            analysis = analyze_ready_tiles(list(tiles_tl), hand)
                            annotated_tiles = []
                            for tile in sorted(tiles_tl, key=lambda x: (x not in SYMBOLS, x)):
                                if tile in analysis:
                                    annotated_tiles.append(analysis[tile]['display'])
                                else:
                                    annotated_tiles.append(str(tile))
                            ready_info['天龙'] = annotated_tiles
                        else:
                            ready_info['天龙'] = sorted(tiles_tl,
                                                        key=lambda x: (x not in SYMBOLS, x))
                
                # 检查地龙听牌
                is_ready_dl, tiles_dl = self.special_winning_checker.is_ready_di_long(hand)
                if is_ready_dl:
                    if return_details:
                        dl_details = {}
                        for tile in tiles_dl:
                            test_hand = hand + [tile]
                            can_win, _ = self.special_winning_checker.can_win_di_long(test_hand)
                            if can_win:
                                fan_info = self._calculate_fan(test_hand, [], "地龙", None)
                                dl_details[tile] = {
                                    'groups': [],
                                    'win_type': '地龙',
                                    'fan_info': fan_info
                        ...
                        ready_info['地龙'] = {
                            'tiles': sorted(tiles_dl, key=lambda x: (x not in SYMBOLS, x)),
                            'details': dl_details
                        }
                    else:
                        # 使用空听检查模块分析听牌状态
                        if EMPTY_LISTENING_AVAILABLE:
                            analysis = analyze_ready_tiles(list(tiles_dl), hand)
                            annotated_tiles = []
                            for tile in sorted(tiles_dl, key=lambda x: (x not in SYMBOLS, x)):
                                if tile in analysis:
                                    annotated_tiles.append(analysis[tile]['display'])
                                else:
                                    annotated_tiles.append(str(tile))
                            ready_info['地龙'] = annotated_tiles
                        else:
                            ready_info['地龙'] = sorted(tiles_dl,
                                                        key=lambda x: (x not in SYMBOLS, x))
